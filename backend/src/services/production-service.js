...
const notificationService = require('./notification-service'); // Imported Notification Service
const { v4: uuidv4 } = require('uuid'); // NEW: Required for generating transaction IDs
...
// ============================================================================
// MASSIVE ERP WIN: GLOBAL INVENTORY SYNC FOR QA GATES
// ============================================================================
const approveQAGate = async (batchId, gateId, userId, destinationLocationId = null) => {
  ...
  try {
    ...
    if (gateNumber === 1) {
      ...
      // AUTO-FALLBACK: Guarantee we find a location to save to, even if the UI forgets to send one
      let targetLocationId = destinationLocationId;
      if (!targetLocationId) {
         const locRes = await client.query("SELECT location_id FROM warehouse_locations WHERE location_type = 'finished_goods' AND is_active = true LIMIT 1");
         if (locRes.rows.length > 0) targetLocationId = locRes.rows[0].location_id;
         else {
            const anyLoc = await client.query("SELECT location_id FROM warehouse_locations WHERE is_active = true LIMIT 1");
            if (anyLoc.rows.length > 0) targetLocationId = anyLoc.rows[0].location_id;
         }
      }

      // Fetch Transaction Types to properly register the movement
      const txTypesRes = await client.query("SELECT type_name, transaction_type_id FROM transaction_types WHERE type_name IN ('receive', 'issue')");
      const txTypes = {};
      txTypesRes.rows.forEach(r => txTypes[r.type_name] = r.transaction_type_id);

      // 1. Get Batch output stats AND Product's Base UOM
      const batchDetails = await client.query(
        `SELECT pb.product_id, pb.batch_number, pb.actual_output, p.base_uom 
         FROM production_batches pb 
         LEFT JOIN products p ON pb.product_id = p.product_id 
         WHERE pb.batch_id = $1`,
        [batchId]
      );
      const { product_id, batch_number, actual_output, base_uom } = batchDetails.rows[0];
      
      // Default to 'piece' if base_uom is somehow completely null
      const uom = base_uom || 'piece';

      if (actual_output && actual_output > 0 && targetLocationId) {
        
        // 2. Add Finished Goods to the Warehouse
        const invCheck = await client.query(
          'SELECT inventory_id FROM inventory WHERE product_id = $1 AND location_id = $2',
          [product_id, targetLocationId]
        );
        ...
        if (invCheck.rows.length > 0) {
          ...
          await client.query(
            ...
            [product_id, targetLocationId, actual_output, uom]
          );
        }

        // 3. LOG THE CREATION TRANSACTION ('RECEIVE')
        const txNumber = `PRD-RCV-${batch_number.split('-').slice(-2).join('-')}`;
        if (txTypes['receive']) {
            await client.query(
              `INSERT INTO inventory_transactions (
                transaction_id, transaction_number, transaction_type_id, product_id, quantity, uom, 
                to_location_id, transaction_date, performed_by, reference_document_number, status, created_at
              ) VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP, $8, $9, 'completed', CURRENT_TIMESTAMP)`,
              [uuidv4(), txNumber, txTypes['receive'], product_id, actual_output, uom, targetLocationId, userId, batch_number]
            );
        }

        // 4. Deduct and log consumed Raw Materials ('ISSUE')
        const components = await client.query(
            `SELECT bc.inventory_id, bc.product_id, bc.actual_consumed, bc.planned_quantity, i.location_id, i.uom
             FROM batch_components bc
        ...
        for (const comp of components.rows) {
            ...
            if (consumed > 0) {
                ...
                // Log the depletion transaction
                const issueTxNumber = `PRD-ISS-${Date.now().toString().slice(-6)}`;
                if (txTypes['issue']) {
                    await client.query(
                      `INSERT INTO inventory_transactions (
                        transaction_id, transaction_number, transaction_type_id, product_id, quantity, uom, 
                        from_location_id, transaction_date, performed_by, reference_document_number, status, created_at
                      ) VALUES ($1, $2, $3, $4, $5, $6, $7, CURRENT_TIMESTAMP, $8, $9, 'completed', CURRENT_TIMESTAMP)`,
                      [uuidv4(), issueTxNumber, txTypes['issue'], comp.product_id, consumed, comp.uom || 'piece', comp.location_id, userId, batch_number]
                    );
                }
                
                // Trigger Low Stock Email Checks
                try {
...
const rejectQAGate = async (batchId, gateId, userId, reason) => {
  ...
  try {
    ...
    // ========================================================================
    // TRIGGER EMAIL NOTIFICATION FOR REJECTED BATCH
    // ========================================================================
    try {
      const batchData = await client.query('SELECT batch_number, COALESCE(pb.product_name, p.product_name) as product_name FROM production_batches pb LEFT JOIN products p ON pb.product_id = p.product_id WHERE pb.batch_id = $1', [batchId]);
