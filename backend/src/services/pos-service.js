...
async function searchCustomers(searchTerm) {
  const result = await query(`
    ...
    WHERE status = 'active'
      AND (
        trading_name  ILIKE $1
        OR legal_name ILIKE $1
        OR vtl_customer_id ILIKE $1
        OR email ILIKE $1
      )
...
async function closeSession(sessionId, cashierId, closingCash, notes) {
  ...
  const result = await query(`
    ...
    WHERE session_id = $5
    RETURNING *
  `, [closingCash, expectedCash, variance, notes || null, sessionId]);
...
async function createTransaction(data, cashierId) {
  ...
  for (const line of evaluatedLines) {
    ...
    // Log inventory transaction as 'sale' (or 'issue' if sale not in ENUM)
    try {
      await query(`
        INSERT INTO inventory_transactions (
          transaction_id, transaction_number, transaction_type,
      ...
      // If 'sale' not in ENUM yet, fall back to 'issue'
      await query(`
        INSERT INTO inventory_transactions (
          transaction_id, transaction_number, transaction_type,
          product_id, quantity, uom,
