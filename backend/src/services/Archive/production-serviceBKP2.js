...
const createBatch = async (batchData, userId) => {
  ...
  try {
    ...
    // Generate batch number: PROD-DDMMYY-XXX
    ...
    `;
    const sequenceResult = await client.query(sequenceQuery, [`%-${dateStr}-%`]);
...
const assignComponents = async (batchId, componentAssignments) => {
  ...
  try {
    ...
    console.log(`📦 Assigning ${componentAssignments.length} components to batch ${batchId}`);
    console.log('Component assignments:', JSON.stringify(componentAssignments, null, 2));
    ...
    for (const assignment of componentAssignments) {
      ...
      console.log(`  - Assigning ${inventoryData.product_name} from ${inventoryData.location_name}`);
    ...
    console.log(`✅ Components assigned successfully`);
