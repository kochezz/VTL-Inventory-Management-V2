...
router.get('/stats', authenticate, authorize(['admin', 'manager', 'qa', 'staff', 'viewer', 'operator', 'ceo', 'cfo', 'junior_accountant']), async (req, res) => {
  try {
...
router.get('/active-production', authenticate, authorize(['admin', 'manager', 'qa', 'staff', 'viewer', 'operator', 'ceo', 'cfo', 'junior_accountant']), async (req, res) => {
  try {
    const query = `
      SELECT 
        pb.batch_id, 
...
router.get('/recent-transactions', authenticate, authorize(['admin', 'manager', 'qa', 'staff', 'operator', 'ceo', 'cfo', 'junior_accountant']), async (req, res) => {
  try {
...
router.get('/low-stock-alerts', authenticate, authorize(['admin', 'manager', 'qa', 'staff', 'viewer', 'operator', 'ceo', 'cfo', 'junior_accountant']), async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 10;
    const query = `
      WITH product_stock AS (
...
router.get('/locations', authenticate, authorize(['admin', 'manager', 'qa', 'staff', 'viewer', 'operator', 'ceo', 'cfo', 'junior_accountant']), async (req, res) => {
  try {
    const query = `
