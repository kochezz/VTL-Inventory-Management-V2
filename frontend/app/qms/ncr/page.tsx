...
export default function NCRDashboardPage() {
  ...
  const fetchUsers = async () => {
    try { const res = await api.get('/users'); setUsers(res.data.filter((u: any) => ['qa', 'manager', 'admin'].includes(u.role))); } 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-dark-700">
                {loading ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-400"><div className="w-8 h-8 border-2 border-red-500/30 border-t-red-500 rounded-full animate-spin mx-auto mb-3"></div>Loading NCRs...</td></tr>
                ) : filteredNCRs.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-500">No Non-Conformances found.</td></tr>
                ) : (
                  filteredNCRs.map((ncr) => (
                    <tr key={ncr.ncr_id} onClick={() => handleOpenModal(ncr)} className="hover:bg-dark-700/50 transition-colors cursor-pointer group">
                      <td className="px-6 py-4 whitespace-nowrap"><span className="font-mono text-sm font-bold text-gray-200 group-hover:text-red-400 transition-colors">{ncr.ncr_code}</span><p className="text-xs text-gray-500 mt-1">{new Date(ncr.created_at).toLocaleDateString()}</p></td>
                      <td className="px-6 py-4 whitespace-nowrap"><span className="text-sm font-bold text-white bg-dark-900 px-2 py-1 rounded border border-dark-600">{ncr.source_module}</span><p className="text-xs text-gray-400 font-mono mt-1">{ncr.source_id || 'No Ref'}</p></td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-start pt-10 overflow-y-auto">
          <div className="bg-dark-800 border border-dark-700 rounded-xl w-full max-w-3xl shadow-2xl mb-10">
            <div className="px-6 py-4 border-b border-dark-700 bg-dark-900 sticky top-0 z-10 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white flex items-center gap-2"><AlertOctagon className={`w-6 h-6 ${selectedNcr ? 'text-blue-400' : 'text-red-500'}`}/>{selectedNcr ? `Update ${selectedNcr.ncr_code}` : 'Raise Non-Conformance (NCR)'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div><label className="block text-sm text-gray-300 mb-1">Severity Level</label><select value={formData.severity} onChange={e => setFormData({...formData, severity: e.target.value})} className="w-full px-4 py-2 bg-dark-950 border border-dark-600 rounded-lg text-white focus:border-blue-500"><option value="Minor">Minor (Easily corrected)</option><option value="Major">Major (Product loss, moderate risk)</option><option value="Critical">Critical (Food safety risk, recall risk)</option></select></div>
                  <div><label className="block text-sm text-gray-300 mb-1">Assign Investigator</label><select value={formData.assigned_to} onChange={e => setFormData({...formData, assigned_to: e.target.value})} className="w-full px-4 py-2 bg-dark-950 border border-dark-600 rounded-lg text-white focus:border-blue-500"><option value="">-- Unassigned --</option>{users.map(u => <option key={u.user_id} value={u.user_id}>{u.full_name} ({u.role})</option>)}</select></div>
                </div>
                {selectedNcr && (
                  <>
                    <div className="mb-4"><label className="block text-sm text-gray-300 mb-1">Root Cause Analysis</label><textarea value={formData.root_cause} onChange={e => setFormData({...formData, root_cause: e.target.value})} rows={3} className="w-full px-4 py-2 bg-dark-950 border border-dark-600 rounded-lg text-white focus:border-blue-500" placeholder="Why did this happen?..." /></div>
                    <div className="mb-4"><label className="block text-sm text-gray-300 mb-1">Immediate Correction / Resolution</label><textarea value={formData.resolution} onChange={e => setFormData({...formData, resolution: e.target.value})} rows={3} className="w-full px-4 py-2 bg-dark-950 border border-dark-600 rounded-lg text-white focus:border-blue-500" placeholder="What was done immediately to fix the issue?..." /></div>
                    <div className="bg-dark-900 p-4 rounded-lg border border-dark-600">
                      ...
                      {formData.status === 'CAPA_REQUIRED' && <p className="text-xs text-purple-400 mt-2 flex items-center gap-1"><ShieldAlert className="w-4 h-4"/> Saving this will flag the NCR for the formal CAPA workflow.</p>}
                    </div>
                  </>
                )}
              </div>
