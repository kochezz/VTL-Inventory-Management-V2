...
export default function BatchDetailPage() {
  ...
  // Added 'user' to the destructuring here so the role checks work
  ...
  const handleQAApproval = async (batchId: string, gateId: string, action: 'approve' | 'reject') => {
    try {
      ...
      if (err.response?.status === 403) {
        setError('You do not have permission to approve/reject QA gates.');
      } else {
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-white font-semibold">
                        {batch.yield_percentage ? parseFloat(batch.yield_percentage as string).toFixed(1) : '0.0'}%
                      </p>
                    </div>
                  </>
                )}
              ...
              <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-white">IPQC Records</h3>
                      <p className="text-sm text-gray-400">In-process quality checks every 30 minutes</p>
                    </div>
                  </div>
                  <button
                ...
                {nextIPQCDue && (
                  <div className={`p-4 rounded-lg mb-4 ${
                    nextIPQCDue.is_due ? 'bg-red-500/10 border border-red-500/20' : 'bg-blue-500/10 border border-blue-500/20'
                  }`}>
                    <div className="flex items-center gap-3">
                      {nextIPQCDue.is_due ? <AlertCircle className="w-5 h-5 text-red-400" /> : <Clock className="w-5 h-5 text-blue-400" />}
                      <p className={nextIPQCDue.is_due ? 'text-red-400' : 'text-blue-400'}>{nextIPQCDue.message}</p>
                    </div>
                  </div>
                )}
                ...
                {ipqcHistory.length > 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full min-w-max">
                        <thead className="bg-dark-800">
                          <tr>
                            <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Check #</th>
                            <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Time</th>
                            <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Fill (ml)</th>
                            <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Status</th>
                            <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">QA Status</th>
                            {(user?.role === 'admin' || user?.role === 'qa') && (
                              <th className="px-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider bg-blue-500/10">Actions</th>
                            )}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-dark-700">
                          {ipqcHistory.map((check) => (
                            <tr key={check.ipqc_id} className="hover:bg-dark-800/50">
                              <td className="px-3 py-4 text-sm font-medium text-white">#{check.check_sequence}</td>
                              <td className="px-3 py-4 text-sm text-gray-300">
                                {new Date(check.check_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                              </td>
                              <td className="px-3 py-4 text-sm text-gray-300">{check.fill_volume_ml}</td>
                              <td className="px-3 py-4">
                                <span className={`px-2 py-1 rounded text-xs ${check.all_checks_passed ? 'text-green-400 bg-green-400/10' : 'text-red-400 bg-red-400/10'}`}>
                                  {check.all_checks_passed ? 'PASS' : 'FAIL'}
                                </span>
                              </td>
                              <td className="px-3 py-4">
                                <span className="text-xs text-gray-300">{check.qa_status}</span>
                              </td>
                              {(user?.role === 'admin' || user?.role === 'qa') && (
                                <td className="px-3 py-4">
                                  <button 
                  ...
                  <div className="text-center py-8 text-gray-500">No IPQC records yet</div>
                )}
              </div>
            )}
          </div>
