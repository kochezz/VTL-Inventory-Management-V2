...
{/* IPQC Records Section - Only show when in production */}
  ...
  <tr className="border-b border-dark-700">
    ...
    <th className="text-left text-sm font-medium text-gray-400 pb-3">QA Status</th>  {/* NEW */}
    <th className="text-left text-sm font-medium text-gray-400 pb-3">Operator</th>
    <th className="text-left text-sm font-medium text-gray-400 pb-3">Actions</th>    {/* NEW */}
  </tr>
</thead>
                    <tbody>
    ...
    <tr key={check.ipqc_id} className="border-b border-dark-700/50">
      ...
      <td className="py-3">
        {check.qa_status === 'qa_approved' ? (
          ...
          <span className="px-2 py-1 bg-yellow-500/10 text-yellow-400 rounded text-xs flex items-center gap-1 w-fit">
      ...
      <td className="py-3">
        ...
        {check.qa_status === 'qa_rejected' && (
          <button
            onClick={() => handleOpenQAReview(check.ipqc_id)}
            className="px-3 py-1 bg-dark-700 hover:bg-dark-600 text-white rounded text-xs transition-colors"
          >
            View
          </button>
        )}
