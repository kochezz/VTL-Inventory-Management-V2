              ...
              {ipqcHistory.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {ipqcHistory.map((check, index) => (
                    ...
                    >
                      {/* Header Row - Check Info & Overall Status */}
                      <div className="flex items-center justify-between mb-4 pb-3 border-b border-dark-700">
                        <div className="flex items-center gap-4">
                          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-primary-500/10 border border-primary-500/20">
                            <span className="text-primary-400 font-bold text-sm">
                              #{check.check_sequence}
                            </span>
                          </div>
                          <div>
                            <p className="text-white font-semibold text-sm">IPQC Check #{check.check_sequence}</p>
                        ...
                        <div>
                          {check.all_checks_passed ? (
                            <span className="inline-flex items-center gap-2 px-4 py-2 bg-green-500/20 border border-green-500/40 rounded-lg">
                              <CheckCircle2 className="w-4 h-4 text-green-400" />
                              <span className="text-xs font-bold text-green-400 uppercase tracking-wide">Pass</span>
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-2 px-4 py-2 bg-red-500/20 border border-red-500/40 rounded-lg">
                              <XCircle className="w-4 h-4 text-red-400" />
                              <span className="text-xs font-bold text-red-400 uppercase tracking-wide">Fail</span>
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div className="bg-dark-800/50 rounded-lg p-3 border border-dark-700">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs text-gray-400 uppercase tracking-wide">Fill Volume</span>
                        ...
                        <div className="bg-dark-800/50 rounded-lg p-3 border border-dark-700">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs text-gray-400 uppercase tracking-wide">Cap Torque</span>
                      ...
                      {/* Visual Checks - 3 columns */}
                      <div className="grid grid-cols-3 gap-3 mb-4">
                        {/* Visual Inspection */}
                        <div className="bg-dark-800/50 rounded-lg p-3 border border-dark-700 text-center">
                          <p className="text-xs text-gray-400 mb-2 uppercase tracking-wide">Visual</p>
                        ...
                        {/* Coding */}
                        <div className="bg-dark-800/50 rounded-lg p-3 border border-dark-700 text-center">
                          <p className="text-xs text-gray-400 mb-2 uppercase tracking-wide">Code</p>
                      ...
                      {/* Footer - QA Status, Operator, Actions */}
                      <div className="flex items-center justify-between pt-3 border-t border-dark-700">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2 text-gray-400">
                            ...
                            <span className="text-sm">{check.operator_name}</span>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          {check.qa_status === 'pending_qa_review' && (
                            <button
                              ...
                              className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-semibold transition-all hover:scale-105 shadow-lg shadow-primary-500/20"
                            >
                              Review Check
                            </button>
                          )}
                          {check.qa_status === 'qa_approved' && (
                            <button
                              onClick={() => handleOpenQAReview(check.ipqc_id)}
                              className="px-4 py-2 bg-dark-700 hover:bg-dark-600 text-gray-300 rounded-lg text-sm font-semibold transition-all"
                            >
                              View Details
                            </button>
                          )}
                          {check.qa_status === 'qa_rejected' && (
                            <button
                              onClick={() => handleOpenQAReview(check.ipqc_id)}
                              className="px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 rounded-lg text-sm font-semibold transition-all"
                            >
                              View Rejection
                            </button>
                          )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
              ...
              <div className="space-y-3">
                ...
                {batch.status === 'in_progress' && (
                  <>
                    {/* Show Submit for QA if there are DRAFT IPQC checks */}
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1">
                              <p className="text-blue-400 text-sm font-semibold">
                                {ipqcHistory.filter(c => c.qa_status === 'draft_check').length} IPQC check(s) ready to submit
                              </p>
                              <p className="text-blue-400/70 text-xs mt-1">
                                Submit checks for QA review to continue production workflow
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    {/* Show alert if there are PENDING QA checks (already submitted, waiting for QA) */}
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            <p className="text-yellow-400 text-sm font-semibold">
                              {ipqcHistory.filter(c => c.qa_status === 'pending_qa_review').length} IPQC check(s) awaiting QA approval
                            </p>
                    ...
                    <button
                      ...
                      disabled={
                        ...
                        ipqcHistory.some(check => check.qa_status === 'draft_check') ||  // Has draft checks
                        ipqcHistory.some(check => check.qa_status === 'pending_qa_review') ||  // Has pending checks
                      ...
                      title={
                        ipqcHistory.length === 0 
                          ? 'Record at least one IPQC check first'
                          : ipqcHistory.some(check => check.qa_status === 'draft_check') 
                          ? 'Submit IPQC checks for QA review first' 
                          : ipqcHistory.some(check => check.qa_status === 'pending_qa_review')
                          ? 'Wait for QA to approve IPQC checks'
                          : !ipqcHistory.some(check => check.qa_status === 'qa_approved')
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`font-semibold ${
                            ipqcHistory.filter(c => c.qa_status === 'draft_check').length > 0 
                              ? 'text-blue-400' 
                              : 'text-gray-500'
                          }`}>
                            {ipqcHistory.filter(c => c.qa_status === 'draft_check').length}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400 text-sm flex items-center gap-1">
