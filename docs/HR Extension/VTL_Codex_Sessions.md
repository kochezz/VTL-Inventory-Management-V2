# VTL-ERP HR MODULE — CODEX SESSION PROMPTS
# Vilagio Trading Limited | freshDRIP Water Bottling
# Use these prompts sequentially in Codex from your repo root. Each prompt is designed for a Codex coding session.
...
# ============================================================
# CODEX OPERATING RULES — READ BEFORE EVERY SESSION
# ============================================================

These prompts were converted from the Claude Code session plan for Codex.
They preserve the original HR module logic, protected-file constraints, file
boundaries, backend/frontend behavior, verification steps, and commit sequence.

How to use each session in Codex:
1. Open the VTL-ERP repository in your Codex-enabled environment.
2. Run the PRE-FLIGHT command once before Session 1.
3. Start with CODEX_SESSION_0_PROMPT and complete sessions sequentially.
4. Paste only one session prompt into Codex at a time.
5. Let Codex inspect the repo before editing, then review the diff before committing.
6. Do not skip verification commands unless your local environment lacks the needed service.
7. Keep commits small and exactly aligned to each session.

Codex-specific guardrails:
- Make the smallest safe diff needed for the session.
- Preserve all existing system functionality unless a session explicitly requires a change.
- Do not refactor unrelated files.
- Do not modernize CommonJS backend files into ES modules.
- Do not change database schema files; the HR schema is assumed already migrated.
- Do not edit protected inventory, production, QMS, auth, dashboard, login, or product files.
- Before modifying any existing file, read the relevant file in full and follow its existing patterns.
- After every edit, run the requested syntax/build checks and inspect `git diff`.
- If a path differs from the prompt, use the actual path found by the PRE-FLIGHT command while preserving the intended file/function behavior.
- If tests fail because of an unrelated existing issue, document it in the session summary and avoid fixing unrelated systems.
...
This file will be the shared context reference for all future Codex sessions
...
# ============================================================
# CODEX FINAL HANDOFF CHECKLIST
# ============================================================

Before considering the Codex work complete, confirm:
- HR module spec exists and is the single source of truth.
- Backend service, middleware, routes, and server mount are complete.
- users-service.js received only the targeted role, holiday allowance, and stats changes.
- Frontend HR layout, nav item, dashboard, employees, profile, onboarding, reviews, and performance pages build successfully.
- Salary data is visible only to admin and hr_admin.
- Existing holiday_requests behavior is preserved.
- Existing inventory/products/auth/dashboard/login functionality remains untouched.
- Each session has a clean commit using the requested commit message format.
